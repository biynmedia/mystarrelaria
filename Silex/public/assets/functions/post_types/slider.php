...
function slider_score_meta_box( $object, $box ) {
	...
	?><div class="ci-cf-wrap"><?php
		ci_metabox_open_tab( '' );
			...
			ci_metabox_guide( __( 'If there is text assigned to the following field a button will also appear under the slider title linking to the above URL (if any).', 'ci_theme' ) );
			ci_metabox_input( 'ci_cpt_slider_button_text', __( 'Button text', 'ci_theme' ), array( 'esc_func' => 'esc_html' ) );

			ci_metabox_guide( sprintf( __( 'The video must be hosted in one of the <a href="%s">WordPress supported providers</a>.', 'ci_theme' ), 'https://codex.wordpress.org/Embeds' ) );
...
function slider_save_meta( $post_id, $post ) {
	...
	update_post_meta( $post_id, 'ci_cpt_slider_button_text', sanitize_text_field( $_POST['ci_cpt_slider_button_text'] ) );
