...
<?php
	...
	function ci_theme_taxonomy_paging_request( $query ) {
		// We don't want to mess with the admin panel, explicit posts_per_page values, and irrelevant pages.
		if ( is_admin() ||
		     isset( $query->query_vars['posts_per_page'] ) ||
		     ! $query->is_main_query() ||
		     ! is_tax( array( 'section', 'artist-category', 'gallery-category', 'video-category' ) )
		) {
			return $query;
		}
