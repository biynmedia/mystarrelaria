...
class CI_Event extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		while ( $event->have_posts() ) : $event->the_post();
			...
			$event_address  = get_post_meta( get_the_ID(), 'ci_cpt_events_address', true );
			...
			<li class="group" itemprop="event" itemscope itemtype="http://schema.org/Event">
				<meta itemprop="startDate" content="<?php echo esc_attr($event_date[0] .'-' . $event_date[1] . '-' . $event_date[2]); ?>">
				...
				<div class="tour-group group">
					...
					<div class="tour-place">
						<span class="sub-head" itemprop="name"><?php if ( ! empty( $post->post_content )): ?><a itemprop="url" href="<?php the_permalink(); ?>" <?php echo $event_new_win; ?>><?php endif; ?><?php the_title(); ?><?php if ( ! empty( $post->post_content )): ?></a><?php endif; ?></span>
						<span itemprop="location" itemscope itemtype="http://schema.org/Place" class="main-head">
							<span itemprop="name"><?php echo $event_venue; ?></span>
							<meta itemprop="address" content="<?php echo esc_attr($event_address); ?>">
						</span>
