...
function ci_register_theme_scripts()
{
	...
	wp_register_script( 'soundmanager-core', get_child_or_parent_file_uri( '/js/soundmanager2/script/soundmanager2-nodebug-jsmin.js' ), array( 'jquery' ), '2.97a', true );
	...
	wp_register_script( 'ci-post-edit-scripts', get_child_or_parent_file_uri( '/js/post-edit-scripts.js' ), array(
		'jquery',
		'wp-color-picker',
	), CI_THEME_VERSION, true );
...
function ci_enqueue_theme_scripts()
{
	...
	$params['mapStyles'] = json_decode( ci_setting( 'map_styles' ) );
